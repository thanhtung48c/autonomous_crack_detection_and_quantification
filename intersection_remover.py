        ...
        for i,point in enumerate(points):
            ...
            while True:
                ...
                if len(intersections) >= 2*degrees[i]:
                    ...
                    binary_img[r-radius:r+radius+1,c-radius:c+radius+1] -= morphology.disk(radius) * 255
                    skeleton_img[r-radius:r+radius+1,c-radius:c+radius+1] -= morphology.disk(radius) * 1
                    break
                else:
                    radius += 1
                    
    return (binary_img > 1).astype(np.uint8) , (skeleton_img > 0).astype(np.uint8)
...
if __name__ == "__main__":
    folder_path = "./data"
    mask_paths = sorted(glob.glob(os.path.join(folder_path,'field_data','*png')))
    mask_files = []
    ...
    skeleton_img, graph = skel_pruning_DSE(skel, dist, 40,return_graph=True)
